...
--    - start Silence: how much time between the first item start and the region start (use numbers above 0)
--    - end Silence: how much time between the end of the last item and region end (use numbers above 0)
--    - space in between: space between each item group of overlapping items (use numbers above 0)
--    - Lock items: write a "y" to lock the items after adjusting the position or anything else to leave it empty
--    - Move envelope with items: if "y", moves envelopes on tracks of selected items with them. If "n", doesn't move envelopes with items. If anything else, uses current setting of REAPER's in-built option "move envelopes with media items". *The Script copies the whole envelope with the items, resulting in copying the whole envelope and not just underlying points.*
--    - Move parent envelope(s): if "y", moves envelopes of any parent tracks (except the master track), with the selected items, to keep a designed sound completely intact.
--    - Prev. - next envelope weight (%): As to match the "space in between" variable, time between the item groups gets either inserted or removed. This variable adjusts the point, where this happens between the overlapping item groups - so 0 mean right after the previous item group ends (potentially cutting off the end of its envelope) and 100 meaning right at the start of the next item group (potentially cutting off an envelope fading in)
--    - Isolate items across tracks: if "y", move selected items, so they don't overlap with any other items on other Tracks. If anything else, only overlaps with non-selected items on the same tracks as the selected ones are avoided.
--    
--
...
function userInputValues()
  local continue, s = reaper.GetUserInputs("ReaGlue Region", 8,
          "Start silence:,Space in between:,End silence:,Lock items (y/n):,Move envelope with items (y/n):,Move parent envelope(s) (y/n):,Prev. - next envelope weight (%):,Isolate items across tracks (y/n):,extrawidth=1", 
          "100,1000,50,y,y,y,80,y")
  ...
  -- Convert from CSV string to table (converts a single line of a CSV file)
    ...
    local d3 = q[3]
    local lock1 = q[4]
    local moveEnvelopes1 = q[5]
    local moveEnvelopesParent_TEMP = q[6]
    local envelopeWeighting_TEMP = q[7]
    local isolateItems_TEMP = q[8]
    ...
    envelopeWeighting = envelopeWeighting_TEMP * 0.01
    envelopeWeighting = math.max(0, math.min(envelopeWeighting, 1)) -- limit to 0 to 1
    moveEnvelopesParents = moveEnvelopesParent_TEMP == "y"
    
    isolateItems = isolateItems_TEMP == "y"
