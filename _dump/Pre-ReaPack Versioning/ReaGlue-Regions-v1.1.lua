...
function adjustItemPositions()
    ...
    -- Process items for each track; crossfaded items count as one
    for track, items in pairs(t) do
        local newPosition = r.GetMediaItemInfo_Value(items[1][1], "D_POSITION")
        local crossfadeadjust = 0
        local summednewPos = 0
        ...
        for i = 1, #items do
              ...
              if items[i+1] then
                nextItem = items[i+1][1]
              end
              local itemEnd = newPosition + itemLength
              local origposition =  reaper.GetMediaItemInfo_Value(item, "D_POSITION")
              local origitemEnd = origposition + itemLength
              reaper.SetMediaItemInfo_Value(item, "D_POSITION", newPosition + crossfadeadjust)
              summednewPos = reaper.GetMediaItemInfo_Value(item, "D_POSITION")
              itemEnd = summednewPos + itemLength
               
              if nextItem then
                  local nextItemStart = reaper.GetMediaItemInfo_Value(nextItem, "D_POSITION")
                 
                  if origitemEnd > nextItemStart then
                      crossfadeadjust = summednewPos - origposition 
                      newPosition = nextItemStart
                  else
                      newPosition = itemEnd + d
                      crossfadeadjust = 0
                  end
              else
                  newPosition = itemEnd + d
