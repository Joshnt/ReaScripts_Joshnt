-- @description ReaGlue adaption to Regions (no Input box)
-- @version 3.3
-- @changelog 
...
--    - Lock items: use bool-values to lock the items after adjusting the position
...
--    - Move parent envelope(s): use bool-values to move envelopes of any parent tracks (except the master track), with the selected items, to keep a designed sound completely intact.
--    - Prev. - next envelope weight (%): As to match the "space in between" variable, time between the item groups gets either inserted or removed. This variable adjusts the point, where this happens between the overlapping item groups - so 0 mean right after the previous item group ends (potentially cutting off the end of its envelope) and 1 meaning right at the start of the next item group (potentially cutting off an envelope fading in)
--    - Isolate items across tracks: if true move selected items, so they don't overlap with any other items on other Tracks. If false, only overlaps with non-selected items on the same tracks as the selected ones are avoided.
--    - openRegionDialog: bool-values to set if - after creating the region - REAPER's "Edit Region" Dialog appears
--    
--
