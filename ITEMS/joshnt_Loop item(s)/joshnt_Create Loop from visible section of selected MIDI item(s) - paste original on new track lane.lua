  ...
  for i, sGUID in ipairs(itemGUID_list) do
    ...
    if take then
      ...
      if reaper.GetMediaSourceType(source, '') == 'MIDI' then
        ...
        reaper.Main_OnCommand(42647,0) -- insert new track lane at bottom
        local itemOrig = reaper.GetSelectedMediaItem(0,0)
        reaper.SetMediaItemInfo_Value(itemOrig, 'I_FIXEDLANE', trackLaneNum)
        reaper.SetMediaItemInfo_Value(itemOrig, 'D_POSITION',item_in)
        reaper.SetMediaTrackInfo_Value(track, 'C_LANEPLAYS:'..(trackLaneNum), 0)
