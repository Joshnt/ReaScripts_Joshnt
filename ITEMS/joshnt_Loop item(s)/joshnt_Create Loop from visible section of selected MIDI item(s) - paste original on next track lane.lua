...
if count > 0 then
  ...
  for i, sGUID in ipairs(itemGUID_list) do
    ...
    if take then
      ...
      if reaper.GetMediaSourceType(source, '') == 'MIDI' then
        ...
        local oldLaneInt = reaper.GetMediaItemInfo_Value(item, 'I_FIXEDLANE')
        local track = reaper.GetMediaItemTrack(item)
        local trackLaneNum = reaper.GetMediaTrackInfo_Value(track, 'I_NUMFIXEDLANES')
        reaper.Main_OnCommand(42398, 0) -- Paste
        local itemOrig = reaper.GetSelectedMediaItem(0,0)
        reaper.SetMediaItemInfo_Value(itemOrig, 'I_FIXEDLANE', oldLaneInt+1)
        reaper.SetMediaItemInfo_Value(itemOrig, 'D_POSITION',item_in)
        if oldLaneInt+2 > trackLaneNum then 
          reaper.SetMediaTrackInfo_Value(track, 'C_LANEPLAYS:'..(oldLaneInt+1), 0)
          reaper.MB(oldLaneInt.." and "..trackLaneNum,"",0)
        end
  ...
  reaper.SetEditCurPos(editCurPos, false, false) -- restore previous edit cur pos
