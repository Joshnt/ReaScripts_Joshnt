-- @noindex
-- useful for finding the "where does that come from"
...
local function selectEarliestOverlappingItem(timeInput)
  local itemToSelect = {};
  ...
  for i = 0, reaper.CountSelectedMediaItems(0)-1 do
    ...
    if itemTemp then
      ...
      if start_pos_TEMP <= timeInput and end_pos_TEMP > timeInput and itemToSelectStart >= start_pos_TEMP then
  ...
  if itemToSelect[1] ~= nil then
    ...
    joshnt.TooltipAtMouse("No item overlapping with cursor")
    return nil
  end
end
...
local function main()
  ...
  local itemStart = selectEarliestOverlappingItem(cursorPos)
  if itemStart then
...
main()
reaper.Undo_EndBlock("Select earliest overlapping item from cursor across tracks", -1)
reaper.PreventUIRefresh(-1)
