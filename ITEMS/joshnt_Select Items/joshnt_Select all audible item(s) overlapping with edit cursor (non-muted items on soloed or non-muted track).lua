...
local function selectAllOverlappingItem(timeInput)
  ...
  local soloTrackTable = joshnt.getSoloedTracks()
  local muteTrackTable = joshnt.getMutedTracks()
  local checkSolo = #soloTrackTable > 0
  local checkMute = #muteTrackTable > 0
  ...
  for i = 0, reaper.CountSelectedMediaItems(0)-1 do
    ...
    if itemTemp then
      local track_TEMP = reaper.GetMediaItemTrack(itemTemp)
      local skip = false
      if checkSolo then 
        skip = joshnt.tableContainsVal(soloTrackTable, track_TEMP) == false
      elseif checkMute then
        skip = joshnt.tableContainsVal(muteTrackTable, track_TEMP)
      end
...
main()
reaper.Undo_EndBlock("Select all audible items overlapping with cursor", -1)
reaper.PreventUIRefresh(-1)
