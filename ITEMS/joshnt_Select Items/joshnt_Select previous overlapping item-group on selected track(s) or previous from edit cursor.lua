...
local function main()
  ...
  if numItems == 0 then 
    ...
    for i = #itemStarts, 1, -1 do
      ...
      if (itemStarts[i] <= startPositionSelection and itemEnds[i] >= endPositionSelection) then -- item selection is part of existing group
        ...
        break
      elseif itemEnds[i] < startPositionSelection then
  ...
  if not groupToSelect then 
    joshnt.TooltipAtMouse("No previous overlapping item group found") 
    groupToSelect = 1
  end
...
main()
reaper.Undo_EndBlock("Select previous overlapping item group on selected track", -1)
reaper.PreventUIRefresh(-1)
