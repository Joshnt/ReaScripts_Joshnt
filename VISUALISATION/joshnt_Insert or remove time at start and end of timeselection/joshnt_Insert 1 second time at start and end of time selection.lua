...
local timeAdded_USER = 1 -- as script name suggest 1 second added per script execution; change to your liking
...
if ignoreItemsAtBounds_USER ~= 2 then
  ...
  reaper.GetSet_LoopTimeRange(true, false, startTime-0.01, startTime, false)
  ...
  reaper.SelectAllMediaItems(0, false)
  reaper.GetSet_LoopTimeRange(true, false, endTime, endTime+0.01, false)
  reaper.Main_OnCommand(40717,0) -- select all items in timeselection
  if reaper.CountSelectedMediaItems(0) > 0 then
    itemsBetween = true
  end
...
reaper.Main_OnCommand(40200, 0) -- insert Time
reaper.GetSet_LoopTimeRange(true, false, startTime, startTime + timeAdded_USER, false)
reaper.Main_OnCommand(40200, 0) -- insert Time
reaper.GetSet_LoopTimeRange(true, false, startTime + timeAdded_USER, endTime + timeAdded_USER, false)

reaper.PreventUIRefresh(-1) 
reaper.UpdateArrange()
reaper.Undo_EndBlock('Add Time at borders of time selection', -1)