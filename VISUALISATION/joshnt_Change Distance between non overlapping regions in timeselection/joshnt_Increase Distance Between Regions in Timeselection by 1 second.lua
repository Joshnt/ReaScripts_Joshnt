...
-- @author Joshnt
-- @about
--    Moves region further apart - only use when no items between regions, as those may get ripped apart
-- @changelog
--  + init
...
local timeAddedBetweenRegions_USER = 1 -- as script name suggest 1 second added per script execution; change to your liking
...
for i = 1, #regionOverlapGroups do
  ...
  if i < #regionOverlapGroups then
    center_between_Regions = regionOverlapGroupsBounds[i][2] + ((regionOverlapGroupsBounds[i+1][1] - regionOverlapGroupsBounds[i][2])*0.5)
  end
...
reaper.Undo_EndBlock('Increase Region Distance', -1)