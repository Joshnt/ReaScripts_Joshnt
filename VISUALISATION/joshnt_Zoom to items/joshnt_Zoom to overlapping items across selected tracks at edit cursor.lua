...
local function main()
    ...
    -- adjust groups to selected tracks
    reaper.SelectAllMediaItems(0, false)
    joshnt.reselectItems(itemGroups[groupToSelect])
    reaper.PreventUIRefresh(-1)
    reaper.Main_OnCommand(reaper.NamedCommandLookup("_SWS_TOGTRACKSEL"),0) -- invert track selection
    reaper.Main_OnCommand(reaper.NamedCommandLookup("_SWS_UNSELONTRACKS"),0) -- unselect items on selected tracks
    reaper.Main_OnCommand(reaper.NamedCommandLookup("_SWS_TOGTRACKSEL"),0) -- invert track selection


    -- zoom to groups
    local startPositionSelection, _ = joshnt.startAndEndOfSelectedItems()
    ...
    if moveCursor then
        reaper.SetEditCurPos(startPositionSelection, true, true)
    end
