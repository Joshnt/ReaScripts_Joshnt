...
function userInputValues()
    local continue, s = reaper.GetUserInputs("Regions user input", 8,
        "Time before item group start:,Time after item group end:,Space between regions:,Lock items (y/n):,Region name:,Mother Region name:,Region Color ( [P] ick / default ):,M. Reg. Color ( [P] ick / default ):,extrawidth=100",
        "0,0,2000,y,_[$incr1],,,")
    ...
    local motherRegionName = q[6]
    local chooseColor = q[7]
    local chooseColorMother = q[8]
...
function setMotherRegion()
  selectOriginalSelection(true)
  local startTime_TEMP, endTime_TEMP = startAndEndOfSelectedItems()
  reaper.AddProjectMarker2(0, true, startTime_TEMP-start_silence-0.2, endTime_TEMP+end_silence + 0.2, motherRegionName_GLOBAL, -1, regionColorMother)
end
