...
local function main() 
  ...
  local targetRegion, reg_Start, reg_End, _, _, reg_Name = joshnt.getAllOverlappingRegion(timeRangeStart, timeRangeEnd) -- get regions in time selection
  if #targetRegion == 0 then
    reaper.MB("No regions found in time selection!","joshnt_Error",0)
    return
  end

  for i = 1, #targetRegion do
    reaper.DeleteProjectMarker(0, targetRegion[i], true)
