...
--
local indexMonitoringFX = 1 -- index of Monitoring FX Starting at 0 -> Monitor FX 1 = index 0
--
...
local function main()
    -- Find the FX window (if already open)
    local hwnd_fx = reaper.JS_Window_Find("Monitoring ["..tostring(indexMonitoringFX+1), false)
    -- Check if the FX window was found
    if hwnd_fx then
        -- Bring the FX window to focus
        reaper.JS_Window_SetForeground(hwnd_fx)
    else
      -- Get the master track
      local master_track = reaper.GetMasterTrack(0)
      local fx_handle = 0x1000000 + indexMonitoringFX -- Special index for monitoring FX

      -- Check if there are at least 5 Monitoring FX on the master track
